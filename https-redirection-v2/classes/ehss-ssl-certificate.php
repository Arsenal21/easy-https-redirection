...
class EHSSL_SSL_Certificate
{
        ...
        try {
            ...
            foreach ($authorizations as $authorization) {
                ...
                if (!$client->selfTest($authorization, Afosto\Acme\Client::VALIDATION_HTTP)) {
                    ...
                    throw new \Exception(__('Could not verify ownership via HTTP', EHSSL_TEXT_DOMAIN));
                }
            }
            ...
            return new WP_Error("1003", __("SSL Certificate installation failed. Check the logs for details.", EHSSL_TEXT_DOMAIN));
        } catch (Exception $ex) {
    ...
    public static function get_certificate_urls()
    {
        ...
        if (!file_exists($certificate_file) || !file_exists($private_key_file) || !file_exists($ca_bundle) || !file_exists($certificate_expiry_file)) {
            return new WP_Error('file_not_found', __('Certificate or private key file not found. Please generate a certificate first!', EHSSL_TEXT_DOMAIN));
        }
    ...
    private function create_directories($acme_challenge_dir_path, $certificate_dir_path)
    {
        ...
        if (!is_dir($acme_challenge_dir_path)) {
            if (!mkdir($acme_challenge_dir_path, 0755, true)) {
                ...
                return new WP_Error("1001", __("Failed to create the acme-challenge directory", EHSSL_TEXT_DOMAIN));
            }
        }
        ...
        if (!is_dir($certificate_dir_path)) {
            if (!mkdir($certificate_dir_path, 0755, true)) {
                ...
                return new WP_Error("1002", __("Failed to create the certificate directory", EHSSL_TEXT_DOMAIN));
            }
        }
